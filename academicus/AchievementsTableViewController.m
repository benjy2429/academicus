...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSString *myIdentifier = (isAddCell) ? @"addCell" : @"achievementCell";
    ...
    if (isAddCell) {
        cell.textLabel.text = @"Add new achievement";
    } else {
...
- (void)configureCell:(UITableViewCell *)cell atIndexPath:(NSIndexPath*)indexPath
{
    ...
    cell.detailTextLabel.text = [NSString stringWithFormat:@"Achieved: %@", [formatter stringFromDate:achievement.dateAchieved]];
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (self.isEditing && !self.inSwipeDeleteMode) {
        ...
        if (indexPath.section == 1 && indexPath.row == 0) {
            ...
            [self performSegueWithIdentifier:@"addAchievement" sender:self];
            
        } else if (indexPath.section == 0) {
            // If the user clicks an item cell in edit mode, perform a segue to the edit page
            UITableViewCell *cell = [self.tableView cellForRowAtIndexPath:indexPath];
            [self performSegueWithIdentifier:@"editAchievement" sender:cell];
        }
    }
...
- (void)tableView:(UITableView *)tableView moveRowAtIndexPath:(NSIndexPath *)fromIndexPath toIndexPath:(NSIndexPath *)toIndexPath {
    ...
    Achievement *achievement = [self.fetchedResultsController objectAtIndexPath:fromIndexPath];
    [orderedItems removeObject:achievement];
    [orderedItems insertObject:achievement atIndex:toIndexPath.row];
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (editingStyle == UITableViewCellEditingStyleDelete) {
        ...
        Achievement *achievement = [self.fetchedResultsController objectAtIndexPath:indexPath];
        [self.managedObjectContext deleteObject:achievement];
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    ...
    } else if ([segue.identifier isEqualToString:@"editAchievement"]) {
        UINavigationController *navController = segue.destinationViewController;
        AchievementDetailTableViewController *controller = (AchievementDetailTableViewController*) navController.topViewController;
        controller.delegate = self;
        controller.managedObjectContext = self.managedObjectContext;
